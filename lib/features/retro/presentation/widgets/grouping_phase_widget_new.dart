...
class _GroupingPhaseWidgetState extends State<GroupingPhaseWidget> {
  ...
  void _initializeItems() {
    ...
    final thoughts = viewModel.thoughts;
    
    setState(() {
      _groupItems = thoughts.map((thought) => GroupItem(
        id: thought.id,
        thoughts: [thought],
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          // Auto-layout items in a grid if positions are not set
          for (int i = 0; i < _groupItems.length; i++) {
            if (_groupItems[i].position == Offset.zero) {
              final col = i % 3;
              final row = i ~/ 3;
              _groupItems[i] = _groupItems[i].copyWith(
                position: Offset(
                  col * 200.0 + 20,
                  row * 120.0 + 20,
                ),
