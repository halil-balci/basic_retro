  ...
  List<Widget> _buildDraggableThoughtsList(String category, RetroViewModel viewModel) {
    ...
    return thoughts
        .map((thought) => Container(
              ...
              child: _buildThoughtOrGroup(thought, category, viewModel),
            ))
        .toList();
  }

  Widget _buildThoughtOrGroup(RetroThought thought, String category, RetroViewModel viewModel) {
    ...
    if (groupId != null && _thoughtGroups[groupId] != null && _thoughtGroups[groupId]!.length > 1) {
      // This thought is part of a group with multiple thoughts
      return _GroupCard(
        groupId: groupId,
        thoughts: _thoughtGroups[groupId]!,
        onSplit: () => _splitGroup(groupId),
        viewModel: viewModel,
      );
    } else {
      // This is a single thought
      return _DraggableThoughtCard(
        thought: thought,
        viewModel: viewModel,
...
class _DraggableThoughtCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DragTarget<RetroThought>(
      onWillAccept: (data) {
        return data != null && data.id != thought.id && data.category == thought.category;
      },
